...
import PickupPill from '../components/PickupPill';
...
export default function ServicesPricing({ onOpenAuth }) {
    ...
    return (
        // 🌟 FIX 1: Changed `overflow-x-hidden` to `overflow-x-clip` so Sticky tabs work perfectly!
        <div className="min-h-screen bg-slate-50 font-sans text-[#0F3024] selection:bg-[#E85D04] selection:text-white flex flex-col m-0 p-0 w-full overflow-x-clip">
            {/* <Header /> */}

            <div className="absolute top-0 left-0 w-full h-[600px] bg-[#0F3024] z-0"></div>

            {/* ── HERO SECTION ── */}
            <header className="relative pt-[120px] md:pt-[150px] pb-32 lg:pb-48 bg-transparent w-full m-0 border-none z-10">
                ...
                <div className="relative z-30 max-w-7xl mx-auto px-6 lg:px-12 text-center flex flex-col items-center">
                ...
                <div className="absolute bottom-0 left-0 w-full overflow-hidden leading-[0] z-10 hidden md:block">
                    <svg viewBox="0 0 1200 120" preserveAspectRatio="none" className="relative block w-full h-[6vw] min-h-[35px] max-h-[70px]" style={{ transform: 'rotate(180deg)' }}>
                        <path d="M1200 120L0 16.48 0 0 1200 0 1200 120z" className="fill-slate-50"></path>
                    </svg>
                </div>
            </header>

            <main className="flex-grow z-10 relative pb-24 w-full mt-20 bg-transparent">
                <div className="max-w-7xl mx-auto px-6 lg:px-12">

                    {/* 🌟 RELATIVE WRAPPER for Sticky Scrolling Tabs 🌟 */}
                    <div className="relative z-20 -mt-16">

                        {/* ── STICKY TABS ── */}
                        <div className="sticky top-[80px] sm:top-[100px] z-40 bg-slate-50/90 backdrop-blur-md pt-2 pb-6 -mx-6 px-6 sm:mx-0 sm:px-0">
                            {/* 🌟 FIX 2: Optimized spacing and padding for mobile to reduce horizontal scroll */}
                            <div className="flex flex-nowrap sm:flex-wrap items-center sm:justify-center gap-1.5 sm:gap-2 md:gap-4 p-1.5 sm:p-2 bg-white rounded-2xl md:rounded-full shadow-[0_15px_40px_-10px_rgba(15,48,36,0.15)] border border-gray-100 max-w-3xl mx-auto overflow-x-auto hide-scrollbar w-full">
                                {services.map((service) => (
                                    <button
                                        key={service.id}
                                        onClick={() => setActiveTabId(service.id)}
                                        className={`flex items-center gap-1.5 sm:gap-2 px-4 py-2.5 sm:px-6 sm:py-4 rounded-xl md:rounded-full font-black text-xs sm:text-sm md:text-base tracking-wide uppercase transition-all duration-300 whitespace-nowrap flex-shrink-0 justify-center ${activeTabId === service.id
                                            ? 'bg-[#0F3024] text-white shadow-xl shadow-[#0F3024]/30 sm:scale-105'
                                            ...
                                            {(showAllPricing ? activeService.pricing.items : activeService.pricing.items.slice(0, 8)).map((item, idx) => (
                                                <div key={idx} className="flex justify-between items-center py-3 border-b border-gray-50 last:border-0 hover:bg-[#0F3024]/[0.03] px-3 rounded-xl transition-colors group/item cursor-default">
                                                    <div className="flex items-center gap-4">
                                                        <div className="w-10 h-10 rounded-xl bg-[#0F3024]/5 flex items-center justify-center p-2 group-hover/item:scale-110 transition-transform duration-300 border border-gray-100 shadow-sm text-[#0F3024]">
